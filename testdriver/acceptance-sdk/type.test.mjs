...
describe('Type Test', () => {
  ...
  it('should enter standard_user in username field', async () => {
    ...
    const result = await client.assert('the username field contains "standard_user"');
  ...
  it('should show validation message when clicking Sign In without password', async () => {
    ...
    await client.focusApplication('Google Chrome');
    const result = await client.assert('Please fill out this field is visible near the password field');
    expect(result).toBeTruthy();
  });
});
