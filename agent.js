...
const runCommand = async (command, depth, shouldSave, pushToHistory) => {
...
let run = async (
  ...
  for (const step of ymlObj.steps) {
    ...
    if (!step.commands && !step.prompt) {
      ...
      await exploratoryLoop(step.prompt, false, true, shouldSave);
    } else {
      let markdown = `\`\`\`yaml
${yaml.dump(step)}
  \`\`\``;
...
const embed = async (file, depth, pushToHistory) => {
  ...
  for (const step of ymlObj.steps) {
    if (!step.commands && !step.prompt) {
      ...
      await executeCommands(step.commands, depth, pushToHistory);
    }
  }
