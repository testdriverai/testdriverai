...
const exit = async (failed = true, overwrite = false) => {
...
let save = async ({ filepath = thisFile, silent = false, overwrite = false } = {}) => {
...
let run = async (file, overwrite = true, shouldExit = true) => {
...
\`\`\``;
  ...
  if (overwrite || overwrite == "true") {
    await save({ filepath: file, overwrite: true });
  }
