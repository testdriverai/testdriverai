...
class TestDriverReporter {
  ...
  async onTestRunStart() {
    ...
    if (!this.options.apiKey) {
      console.log(
        "[TestDriver Reporter] No API key provided in options, skipping test recording",
      );
      return;
    }
  ...
  async onTestRunEnd(testModules, unhandledErrors, reason) {
    console.log(
      "[TestDriver Reporter] onTestRunEnd called with reason:",
      reason,
    );
    ...
    try {
      ...
      console.log(
        `[TestDriver Reporter] Completing test run ${this.testRunId} with status: ${status}`,
      );
      ...
      console.log(
        `[TestDriver Reporter] Checking for dashcam URLs after test completion...`,
      );
      ...
      try {
        ...
        const dashcamFiles = files.filter(
          (f) => f.startsWith("testdriver-dashcam-") && f.endsWith(".json"),
        );

        console.log(
          `[TestDriver Reporter] Found ${dashcamFiles.length} dashcam temp files in ${tempDir}`,
        );
        ...
        for (const f of dashcamFiles) {
          ...
          try {
            ...
            if (data.sessionId)
              dashcamMap.bySession.set(String(data.sessionId), data);
            if (data.pid) dashcamMap.byPid.set(String(data.pid), data);
            if (data.replayObjectId)
              dashcamMap.byReplayId.set(String(data.replayObjectId), data);
          } catch {
            console.log(
              `[TestDriver Reporter] Failed to parse dashcam temp file ${f}`,
            );
          }
        }
        ...
        if (dashcamMap.entries.length === 0) {
          console.log(
            `[TestDriver Reporter] No readable dashcam temp files found`,
          );
        } else {
          console.log(
            `[TestDriver Reporter] Indexed ${dashcamMap.entries.length} dashcam temp files`,
          );
        ...
        console.log(
          `[TestDriver Reporter] Could not read temp files:`,
          error.message,
        );
      }
      ...
      if (!dashcamUrl) {
        for (const [, testData] of this.testCases.entries()) {
          ...
          if (meta?.testdriverDashcamUrl) {
            ...
            console.log(
              `[TestDriver Reporter] ✓ Found dashcam URL in task.meta: ${dashcamUrl}`,
            );
            break;
          }
        }
      ...
      if (!dashcamUrl && globalThis.__testdriverMeta?.__lastDashcamUrl__) {
        ...
        console.log(
          `[TestDriver Reporter] ✓ Found dashcam URL in global meta: ${dashcamUrl}`,
        );
      }

      // If we indexed dashcam files, try to associate each test with the correct replay
      const anyIndexed =
        typeof dashcamMap !== "undefined" && dashcamMap.entries.length > 0;
      if (anyIndexed || globalThis.__testdriverMeta?.__lastDashcamUrl__) {
        console.log(
          `[TestDriver Reporter] Sending test cases with dashcam data now available...`,
        );
        ...
        for (const [, testData] of this.testCases.entries()) {
          ...
          const testFile =
            test.module?.relativeModuleId || test.file?.name || "unknown";
          ...
          console.log(
            `[TestDriver Reporter] Matching replay for test: ${test.name}`,
          );
          ...
          const meta =
            typeof test.meta === "function" ? test.meta() : test.meta;
          console.log(
            `[TestDriver Reporter]   meta type:`,
            typeof test.meta,
            "keys:",
            meta ? Object.keys(meta) : "none",
          );
          if (meta?.testdriverDashcamUrl) {
            replayUrlForTest = meta.testdriverDashcamUrl;
            console.log(
              `[TestDriver Reporter]   ✓ Found in meta:`,
              replayUrlForTest,
            );
          }
          ...
          if (!replayUrlForTest) {
            try {
              if (globalThis.__testdriverRegistry) {
                console.log(
                  `[TestDriver Reporter]   Checking registry, clients:`,
                  globalThis.__testdriverRegistry.clients?.size,
                );
                const client =
                  globalThis.__testdriverRegistry.getClient?.(test.id) ||
                ...
                if (client && typeof client.getSessionId === "function") {
                  ...
                  console.log(
                    `[TestDriver Reporter]   dashcamMap.bySession has:`,
                    Array.from(dashcamMap.bySession.keys()),
                  );
                  if (dashcamMap.bySession.has(sid)) {
                    replayUrlForTest = dashcamMap.bySession.get(sid).dashcamUrl;
                    console.log(
                      `[TestDriver Reporter]   ✓ Found by sessionId:`,
                      replayUrlForTest,
                    );
                  } else {
                    console.log(
                      `[TestDriver Reporter]   ✗ Session ID not in map`,
                    );
                  }
                } else {
                  console.log(
                    `[TestDriver Reporter]   ✗ Client has no getSessionId method`,
                  );
                }
              } else {
              ...
              console.log(
                `[TestDriver Reporter]   ✗ Error accessing registry:`,
                err.message,
              );
            }
          }
          ...
          if (!replayUrlForTest && dashcamMap.entries.length > 0) {
            console.log(
              `[TestDriver Reporter]   Trying to match by test ID in filename, entries:`,
              dashcamMap.entries.length,
            );
            const rawTaskId = test.id || test.name;
            const safeTaskId = String(rawTaskId).replace(/[^a-z0-9-_]/gi, "_");
            console.log(
              `[TestDriver Reporter]   Looking for taskId:`,
              safeTaskId,
            );
            ...
            for (const entry of dashcamMap.entries) {
              if (entry.file.includes(safeTaskId)) {
                ...
                console.log(
                  `[TestDriver Reporter]   ✓ Found by taskId in filename:`,
                  entry.file,
                );
                break;
              }
            ...
            if (!replayUrlForTest) {
              console.log(
                `[TestDriver Reporter]   ✗ No filename match for taskId`,
              );
            }
          }
          ...
          if (
            !replayUrlForTest &&
            dashcamMap.entries.length === 1 &&
            this.testCases.size === 1
          ) {
            replayUrlForTest = dashcamMap.entries[0].data.dashcamUrl;
            console.log(
              `[TestDriver Reporter]   ✓ Using single replay for single test:`,
              replayUrlForTest,
            );
          }

          console.log(
            `[TestDriver Reporter]   Final replayUrlForTest:`,
            replayUrlForTest || "NONE",
          );
        ...
        console.log(
          `[TestDriver Reporter] No dashcam data found after test completion`,
        );
      }
  ...
  _getPlatform() {
    ...
    if (this.detectedPlatform) {
      console.log(
        `[TestDriver Reporter] Using platform from SDK client: ${this.detectedPlatform}`,
      );
    ...
    if (
      ...
      globalThis.__testdriverRegistry.clients.size > 0
    ) {
      const firstClient = globalThis.__testdriverRegistry.clients
        .values()
        .next().value;
      if (firstClient && firstClient.os) {
        const platform = firstClient.os.toLowerCase();
        console.log(
          `[TestDriver Reporter] Using platform from global registry client: ${platform}`,
        );
  ...
  _detectPlatformFromTest(test) {
    ...
    if (client && client.os) {
      ...
      console.log(
        `[TestDriver Reporter] Detected platform from test context: ${platform}`,
      );
    }
  }
  ...
  async _findReplayForTest(test) {
    console.log(
      `[TestDriver Reporter] ===== Finding replay URL for test: ${test.name} =====`,
    );
    ...
    if (process.env.DASHCAM_REPLAY_URL) {
      console.log(
        `[TestDriver Reporter] Using DASHCAM_REPLAY_URL from env: ${process.env.DASHCAM_REPLAY_URL}`,
      );
    ...
    if (meta && Object.keys(meta).length > 0) {
      ...
      if (meta.testdriverPlatform) {
        console.log(
          `[TestDriver Reporter] ✓ Found platform in task.meta: ${meta.testdriverPlatform}`,
        );
        if (!this.detectedPlatform) {
      ...
      if (meta.testdriverDashcamUrl) {
        console.log(
          `[TestDriver Reporter] ✓ Found dashcam URL in task.meta: ${meta.testdriverDashcamUrl}`,
        );
      ...
      if (meta.replayUrl) {
        console.log(
          `[TestDriver Reporter] Found replayUrl in test.meta: ${meta.replayUrl}`,
        );
        return meta.replayUrl;
      }
      if (meta.dashcamUrl) {
        console.log(
          `[TestDriver Reporter] Found dashcamUrl in test.meta: ${meta.dashcamUrl}`,
        );
        return meta.dashcamUrl;
      }
    } else {
      console.log(
        `[TestDriver Reporter] ⚠️ No task metadata available (meta is empty)`,
      );
    }
    ...
    if (globalThis.__testdriverRegistry) {
      console.log(
        `[TestDriver Reporter] Registry exists with ${globalThis.__testdriverRegistry.clients.size} clients`,
      );
      ...
      if (client) {
        console.log(
          `[TestDriver Reporter] Client has _lastDashcamUrl:`,
          !!client._lastDashcamUrl,
        );
        if (client._lastDashcamUrl) {
          console.log(
            `[TestDriver Reporter] ✓ Found dashcam URL from client registry: ${client._lastDashcamUrl}`,
          );
    ...
    if (globalThis.__testdriverMeta) {
      if (globalThis.__testdriverMeta[test.name]) {
        ...
        console.log(
          `[TestDriver Reporter] ✓ Found test meta in global storage`,
        );
      ...
      if (globalThis.__testdriverMeta.__lastDashcamUrl__) {
        console.log(
          `[TestDriver Reporter] ✓ Found dashcam URL in global meta: ${globalThis.__testdriverMeta.__lastDashcamUrl__}`,
        );
      ...
      if (globalThis.__testdriverMeta.__platform__ && !this.detectedPlatform) {
        ...
        console.log(
          `[TestDriver Reporter] Found platform in global meta: ${this.detectedPlatform}`,
        );
      }
    }
    ...
    const urlPattern =
      /(https?:\/\/(?:app\.)?(?:dashcam\.io|testdriver\.ai|replayable\.io)\/replay\/[a-f0-9]{24}(?:\?[^\s]*)?)/i;
    ...
    const allOutput = [result?.stdout, result?.stderr, ...(result?.logs || [])]
      .filter(Boolean)
      .join("\n");

    console.log(
      `[TestDriver Reporter] Searching for replay URL in test output (${allOutput.length} chars)`,
    );
    ...
    if (allOutput && allOutput.length > 0) {
      ...
      if (allOutput.length < 200) {
        ...
        console.log(
          `[TestDriver Reporter] Output sample (first 200 chars): ${allOutput.substring(0, 200)}...`,
        );
      }
      ...
      if (match) {
        ...
        console.log(
          `[TestDriver Reporter] ✓ Found replay URL in test output: ${replayUrl}`,
        );
        ...
        if (replayIdMatch) {
          console.log(
            `[TestDriver Reporter] ✓ Validated replay ID: ${replayIdMatch[1]}`,
          );
        } else {
          console.warn(
            `[TestDriver Reporter] ⚠ WARNING: Found URL but couldn't validate replay ID format: ${replayUrl}`,
          );
        }
        ...
        console.log(
          `[TestDriver Reporter] ✗ No replay URL pattern match in output`,
        );
      }
    } else {
    ...
    if (test.meta) {
      console.log(
        `[TestDriver Reporter] Checking test.meta:`,
        Object.keys(test.meta),
      );
      if (test.meta.replayUrl) {
        console.log(
          `[TestDriver Reporter] Found replayUrl in test.meta: ${test.meta.replayUrl}`,
        );
        return test.meta.replayUrl;
      }
      if (test.meta.dashcamUrl) {
        console.log(
          `[TestDriver Reporter] Found dashcamUrl in test.meta: ${test.meta.dashcamUrl}`,
        );
    ...
    console.log(
      `[TestDriver Reporter] ===== No replay URL found for test: ${test.name} =====`,
    );
    return null;
  }
  ...
  _getGitInfo() {
    ...
    if (process.env.GITHUB_ACTIONS) {
      ...
      if (process.env.CIRCLE_USERNAME)
        info.author = process.env.CIRCLE_USERNAME;
    }
