...
module.exports = async () => {
  ...
  const response = await prompts([
    ...
    {
      type: (_, values) => (values.TD_VM ? "confirm" : null),
      name: 'generateSetup',
      message: 'Create setup.yaml file?'
    },
    {
      type: prev => (prev ? 'select' : null),
      name: 'setupType',
      message: 'Are you testing a web app or desktop',
      choices: [
        { title: 'Web', value: 'web',},
    ...
    {
      type: (_, values) => (values.TD_VM ? null : "confirm"),
      name: "TD_MINIMIZE",
  ...
  const env = Object.entries(response)
    .filter(([key, value]) => key.startsWith("TD_"))
    .map(([key, value]) => `${key}=${value}`)
    .join("\n");
  const append = path.join(process.cwd(), '.env');

  // Create setup.yaml
  const setupFile = path.join(process.cwd(), "setup.yaml");
  if (response.generateSetup) {
    if (!fs.existsSync(setupFile)) {
      fs.writeFileSync(setupFile, response.setupType === "desktop" ? DESKTOP_SETUP : WEB_SETUP);
      logger.info(chalk.dim(`Writing setup.yaml`));
    } else {
      logger.info(chalk.dim(`Setup file already exists, skipping...`));
    }
  }
...
const WEB_SETUP = `
prerun: |
  # Launch chrome and dont show welcome screen
  (google-chrome \
    --disable-fre \
...
const DESKTOP_SETUP = `
prerun: |
  wget https://myapp.com/myapp.exe
  (./myapp.exe >/dev/null 2>&1) &
  sleep 10
`;
