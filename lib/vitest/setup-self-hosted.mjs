...
beforeEach(async (context) => {
  ...
  // Check if TD_OS=windows is set but we're not running on an actual Windows platform
  // This can happen if the user has TD_OS set in their shell profile or .env file
  const actualPlatform = process.platform; // 'darwin', 'linux', 'win32', etc.
  if (actualPlatform !== "win32") {
    throw new Error(
      `[TestDriver] TD_OS is set to "windows" but you're running on platform "${actualPlatform}" (${actualPlatform === "darwin" ? "macOS" : actualPlatform}).\n` +
      `This setup file (setup-self-hosted.mjs) is designed to spawn Windows AWS instances for testing.\n` +
      `\n` +
      `Possible solutions:\n` +
      `  1. Unset TD_OS if you don't need Windows testing: unset TD_OS\n` +
      `  2. Check your shell profile (.bashrc, .zshrc) or .env files for TD_OS=windows\n` +
      `  3. If you need Windows testing with AWS, ensure you're running in the correct environment\n` +
      `\n` +
      `Current environment:\n` +
      `  TD_OS: ${process.env.TD_OS}\n` +
