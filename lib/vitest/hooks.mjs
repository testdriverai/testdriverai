...
/**
 * Batching state for console output forwarding.
 * Collects messages over a short window and sends them as a single
 * combined "output" message per sandbox, reducing WS traffic.
 */
const _outputBatch = {
  queue: [],              // Array of base64-encoded strings
  timeout: null,
  INTERVAL_MS: 100,       // Flush every 100ms
  MAX_SIZE: 20,           // Or when batch reaches 20 messages
};

function _flushOutputBatch() {
  if (_outputBatch.queue.length === 0) return;

  const batch = _outputBatch.queue;
  _outputBatch.queue = [];
  _outputBatch.timeout = null;

  // Decode each base64 entry, join with newline, re-encode once
 ...
 * Messages are batched (100ms / 20 messages) to reduce WS traffic.
 */
function forwardToAllSandboxes(args) {
  ...
  _outputBatch.queue.push(encoded);

  if (_outputBatch.queue.length >= _outputBatch.MAX_SIZE) {
    if (_outputBatch.timeout) {
      clearTimeout(_outputBatch.timeout);
    }
    _flushOutputBatch();
  } else if (!_outputBatch.timeout) {
    _outputBatch.timeout = setTimeout(() => _flushOutputBatch(), _outputBatch.INTERVAL_MS);
  }
}
...
function cleanupConsoleSpy(client) {
  ...
  if (_consoleSpy.activeClients.size === 0) {
    if (_outputBatch.timeout) {
      clearTimeout(_outputBatch.timeout);
    }
    _flushOutputBatch();
