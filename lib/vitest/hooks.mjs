...
/**
 * Check if AWS self-hosted parameters are provided but setup-aws.mjs is not included
 * This helps users who forget to add the setup file to their vitest config
 */
function checkAWSSetupIncluded() {
  const hasAWSParams = process.env.TD_OS === 'windows' && 
                       (process.env.AMI_ID || process.env.AWS_LAUNCH_TEMPLATE_ID);
  const hasExistingIP = process.env.TD_IP; // If IP is provided, they're connecting to existing instance
  const setupAWSLoaded = globalThis.__testdriverAWS !== undefined;
  
  if (hasAWSParams && !hasExistingIP && !setupAWSLoaded) {
    throw new Error(
      `AWS self-hosted parameters detected (TD_OS=windows, AMI_ID, AWS_LAUNCH_TEMPLATE_ID) but ` +
      `'testdriverai/vitest/setup-aws' is not included in your vitest config.\n\n` +
      `Add it to your vitest.config.mjs:\n\n` +
      `  export default defineConfig({\n` +
      `    test: {\n` +
      `      setupFiles: [\n` +
      `        'testdriverai/vitest/setup',\n` +
      `        'testdriverai/vitest/setup-aws'  // <-- Add this line\n` +
      `      ],\n` +
      `    },\n` +
      `  });\n\n` +
      `Or if you're connecting to an existing instance, set TD_IP instead.`
    );
  }
}
...
export function TestDriver(context, options = {}) {
  ...
  // Check if AWS params are provided but setup-aws.mjs is missing
  checkAWSSetupIncluded();
  ...
  // Always auto-connect - commands will lazy-await this promise
  const debugConsoleSpy = process.env.TD_DEBUG_CONSOLE_SPY === 'true';
