...
export function TestDriver(context, options = {}) {
  ...
  const cleanup = async () => {
    ...
    try {
      ...
      // If the sandbox is unresponsive or the WebSocket dropped into reconnection,
      // dashcam.stop()'s exec() can block for 300 s (send timeout) plus up to ~240 s
      // of exponential-backoff reconnection attempts.  Cap it at 30 s so a single
      // failed test never hangs the whole suite.
      const DASHCAM_STOP_TIMEOUT_MS = 30000;
      if (currentInstance._dashcam && currentInstance._dashcam.recording) {
        try {
          const dashcamStopPromise = currentInstance.dashcam.stop();
          // Attach a no-op catch now so that if the timeout fires first and
          // disconnect() later rejects the in-flight send, there is no
          // unhandled-rejection warning from the abandoned promise.
          dashcamStopPromise.catch(() => {});
          const dashcamUrl = await Promise.race([
            dashcamStopPromise,
            new Promise((resolve) =>
              setTimeout(resolve, DASHCAM_STOP_TIMEOUT_MS),
            ),
          ]);
      ...
      // Guard with a timeout so that a stalled auth/connect (e.g. the sandbox
      // server accepted the TCP connection but never responded) does not block
      // cleanup indefinitely.
      if (currentInstance.__connectionPromise) {
        const CONNECTION_WAIT_TIMEOUT_MS = 30000;
        await Promise.race([
          currentInstance.__connectionPromise.catch(() => {}),
          new Promise((resolve) =>
            setTimeout(resolve, CONNECTION_WAIT_TIMEOUT_MS),
          ),
        ]);
      }
