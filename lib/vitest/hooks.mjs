...
// under all pool modes. Under pool:"threads", process.on('SIGINT') and process.exit() are shared
// across ALL worker threads — registering signal handlers from workers would
// stack N handlers (one per thread) and calling process.exit() from any thread
// kills the entire process. Vitest's lifecycle hooks are the only safe way.
...
export function TestDriver(context, options = {}) {
  ...
  // Plugin options are passed directly via the `options` parameter.
  const pluginOptions = {};
    ...
    try {
      ...
      if (currentInstance._dashcam && currentInstance._dashcam.recording) {
        try {
          ...
          // Also register via task.meta for the reporter to pick up
          // (task.meta is serialized per-task by Vitest — safe under all pool modes)
