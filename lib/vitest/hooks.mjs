...
function registerSignalHandlers() {
  ...
  // Expose flag so setup-aws.mjs knows hooks.mjs will handle process.exit()
  globalThis.__testdriverSignalHandlersRegistered = true;
    ...
    await Promise.race([
      Promise.all(
        instances.map(async (inst) => {
          // 1. Stop dashcam recording so the video is saved
          try {
            if (inst._dashcam && inst._dashcam.recording) {
              await inst.dashcam.stop();
            }
          } catch {
            // Ignore dashcam stop errors during signal cleanup
          }
          // 2. Disconnect WebSocket
          try {
            await inst.disconnect();
          } catch {
            // Ignore disconnect errors during signal cleanup
          }
        }),
      ),
      new Promise((resolve) => setTimeout(resolve, 5000)), // 5s max for cleanup
    ]);
    // 3. Terminate any AWS instances (must happen AFTER dashcam.stop)
    if (globalThis.__testdriverAWS?.instances) {
      for (const [testId] of globalThis.__testdriverAWS.instances) {
        try {
          await globalThis.__testdriverAWS.terminateInstance(testId);
        } catch {
          // Ignore termination errors during signal cleanup
        }
      }
    }
  };
