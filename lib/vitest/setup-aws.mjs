...
import { cleanupAllInstances, ensureAWSGlobal } from "./aws-global.mjs";
...
// Register with the thread-safe global (used by hooks.mjs for terminateInstance)
ensureAWSGlobal(testInstances);

// Use Vitest lifecycle hook for cleanup instead of signal handlers.
// This replaces the old process.on("exit"/"SIGINT"/"SIGTERM") handlers which
// are unsafe under pool:"threads" (they stack N handlers across threads
// and call process.exit(), killing all threads).
afterAll(() => {
  cleanupAllInstances();
});
