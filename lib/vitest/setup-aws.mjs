...
// Initialize the thread-safe global AWS object
ensureAWSGlobal(testInstances);

// Use afterAll (per test-file) as a safety net to terminate any orphaned instances.
// This replaces the old process.on("exit"/"SIGINT"/"SIGTERM") handlers which are
// not thread-safe under pool:"threads" (they stack on the shared process object
