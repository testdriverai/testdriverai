...
// The 'exit' handler is the last-resort safety net — it fires whenever
// process.exit() is called (including from hooks.mjs signal handlers).
process.on("exit", cleanupAllInstances);
process.on("SIGINT", () => {
  cleanupAllInstances();
  // Don't call process.exit() here — hooks.mjs registers its own SIGINT
  // handler that does full async cleanup (dashcam.stop, disconnect, AWS
  // terminate) and then calls process.exit(130).
  // If hooks.mjs hasn't registered yet (no tests started), exit directly.
  if (!globalThis.__testdriverSignalHandlersRegistered) {
    process.exit(130);
  }
});
process.on("SIGTERM", () => {
  cleanupAllInstances();
  // Same as SIGINT — let hooks.mjs handle exit after full cleanup.
  if (!globalThis.__testdriverSignalHandlersRegistered) {
    process.exit(143);
  }
});
process.on("uncaughtException", (error) => {
  ...
  cleanupAllInstances();
  // Must exit after uncaught exception — Node won't exit automatically
  // once an uncaughtException handler is registered.
  process.exit(1);
});
