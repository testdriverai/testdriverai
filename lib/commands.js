...
function splitByRepeatedChars(str) {
  if (!str) return [];
  const result = [];
  let start = 0;
...
const click = async (x, y, action = "click") => {
  ...
  emitter.emit(events.mouseClick.start, { x, y, button });
  ...
  if (action !== "hover") {
    ...
    emitter.emit(events.mouseClick.end, { x, y, button });
  }
...
let commands = {
  ...
  // type a string
  // `cpm` is incorrectly referred to as `delay` within generation / schema
  type: async (string) => {
    await redraw.start();
    ...
    if (config.TD_VM) {
      ...
      // this is because the OS has some sort of timeout for repeated characters
      // so we need to check for repeated characters, and split them up with a timeout

      let segments = splitByRepeatedChars(string);
      if (segments.length > 1) {
        for (let index = 0; index < segments.length; index++) {
          const segment = segments[index];
          await robot.typeString(segment);

          if (index < segments.length - 1) {
            // wait for the cpm delay before typing the next segment
            await delay(1000);
          }
        }
      } else {
