...
const scroll = async (direction = "down", amount = 300, method = "mouse") => {
  ...
  switch (direction) {
    case "up":
      if (method === "mouse") {
        await sandbox.getDesktop().scroll(0, amount );
      } else {
        await sandbox.getDesktop().press("pageup");
      }
      await redraw.wait(2500);
      break;
    case "down":
      if (method === "mouse") {
        await sandbox.getDesktop().scroll(0, amount * -1);
      } else {
        await sandbox.getDesktop().press("pagedown");
      }
      await redraw.wait(2500);
      break;
    case "left":
      await sandbox.getDesktop().scroll(amount * -1, 0);
      await redraw.wait(2500);
      break;
    case "right":
      await sandbox.getDesktop().scroll(amount, 0);
      await redraw.wait(2500);
      break;
    default:
...
const click = async (x, y, action = "click") => {
  ...
  sandbox.getDesktop().moveMouse(x, y);
...
const hover = async (x, y) => {
  ...
  await sandbox.getDesktop().moveMouse(x, y, 0.1);
...
let commands = {
  ...
  type: async (string, delay = 500) => {
    ...
    console.log('writing ', string), ' to desktop'

    await sandbox.getDesktop().write(string);
  ...
  "press-keys": async (inputKeys) => {
    ...
    // sandbox.getDesktop().press(keysPressed[0], modsToPress);

    console.log('pressing keys', keysPressed)

    await sandbox.getDesktop().press(keysPressed) 
  ...
  "scroll-until-text": async (
    ...
    if (method === "keyboard") {
      try {
        ...
        await sandbox.getDesktop().press("f", commandOrControl);
        // keyTap will release the normal keys, but will not release modifier keys
        // so we need to release the modifier keys manually
        // robot.keyToggle(commandOrControl, "up");
        // type the text
        // await robot.typeStringDelayed(text, 500);
        await sandbox.getDesktop().write(text);
        await redraw.wait(5000);
        await sandbox.getDesktop().press("escape");
      } catch (e) {
        logger.error("%s", e);
        throw new AiError(
  ...
  exec: async (cli_command, use_stderr = false, silent = false) => {
    ...
    let out = await sandbox.getDesktop().commands.run(cli_command);
    console.log(out);
    return out;
