...
const click = async (x, y, action = "click") => {
  ...
  sandbox.send({type: "moveMouse", x, y });
...
let commands = {
  ...
  "press-keys": async (inputKeys) => {
    ...
    // sandbox.send({type: "press" }); (keysPressed[0], modsToPress);
  ...
  exec: async (cli_command, use_stderr = false, silent = false) => {
    ...
    // }
      ...
      let out = await sandbox.send({type: "commands.run", commands: cli_command });
      return out;
