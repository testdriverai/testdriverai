 ...
 *
 * @example
 * const { CaptchaSolver } = require('./solver.cjs');
 *
 ...
 *   sitekey: 'optional-if-auto-detect-works',
 *   action: 'verify',
 *   autoSubmit: true
 * });
 */
...
class CaptchaSolver {
  ...
  async connect() {
    ...
    this.log('Targets:', targets.map(t => ({ type: t.type, url: t.url })));
    ...
    const result = await this.Runtime.evaluate({
      expression: `
        (function() {
          ...
          // Detect reCAPTCHA v3 from scripts
          ...
          for (const script of scripts) {
            ...
            // Match render calls
            const renderMatch = content.match(/grecaptcha\\.(?:enterprise\\.)?render\\s*\\([^,]*,\\s*\\{[^}]*sitekey\\s*:\\s*['\"]([0-9A-Za-z_-]{40})['\"]/);
   ...
   */
  async solve(options = {}) {
    const { autoSubmit = true, waitForSuccess = true, sitekey, action } = options;
    ...
    // Use provided sitekey or auto-detect
    let captchas = [];
    if (sitekey) {
      captchas = [{
        type: options.type || 'recaptcha_v3',
        sitekey,
        action: action || 'verify'
      }];
      console.log('CAPTCHAS_PROVIDED:', JSON.stringify(captchas));
    } else {
      captchas = await this.detect();
      console.log('CAPTCHAS_DETECTED:', JSON.stringify(captchas));
    }
    ...
    for (const captcha of captchas) {
      console.log('SOLVING:', captcha.type, captcha.sitekey);

      try {
        const solution = await this.solveCaptcha(captcha);
        console.log('TOKEN:', solution.token.substring(0, 50) + '...');
        ...
        console.error('SOLVE_ERROR:', error.message);
        results.push({
          captcha,
          error: error.message,
          injected: false
