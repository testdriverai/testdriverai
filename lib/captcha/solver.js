...
(async () => {
  try {
    ...
    if (!sitekey) {
      throw new Error("Could not auto-detect captcha. Please provide sitekey manually.");
    }
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      const resultResp = JSON.parse(await httpsGet(
        "https://2captcha.com/res.php?key=" + config.apiKey + "&action=get&id=" + requestId + "&json=1"
      ));
