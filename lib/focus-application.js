...
const tmpScript = path.join(os.tmpdir(), "focusWindow.ps1");
fs.writeFileSync(tmpScript, fs.readFileSync(scriptPath, 'utf8'));
...
const runPwsh = (appName, method) => {
  let script = `powershell -ExecutionPolicy Bypass -Command "& { ${tmpScript} '${appName}' '${method}' }"`;
