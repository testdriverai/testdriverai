...
/**
 * Capture a message
 * @param {string} message - Static message string for Sentry grouping
 * @param {string} level - Severity level (info, warning, error)
 * @param {Object} extra - Additional dynamic data to attach
 */
...
function attachLogListeners(emitter) {
  ...
  emitter.on("command:start", (data) => {
    Sentry.logger.info(
      ...
      getLogAttributes({
        ...
        command: data?.command || data?.name || "unknown",
        ...data,
      }),
    );
  });
  ...
  emitter.on("command:error", (data) => {
    Sentry.logger.error(
      ...
      getLogAttributes({
        category: "cli.command",
        errorMessage: data?.message || data?.error || "unknown",
        ...data,
      }),
    );
  });
  ...
  emitter.on("step:start", (data) => {
    Sentry.logger.info(
      ...
      getLogAttributes({
        category: "cli.step",
        step: data?.step || data?.name || "unknown",
      }),
    );
  });
