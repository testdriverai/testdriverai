...
const req = async (path, data, onChunk) => {
  if (!config.TD_API_KEY) {
    logger.warn(
      theme.red(
        "No API token set. Get a key at https://app.testdriver.ai/team",
      ),
    );
    logger.warn(
      theme.red("Then run `testdriverai init --TD_API_KEY <your_api_key>`"),
    );
    process.exit(1);
  }
  ...
  const c = {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`, // Add the authorization bearer token only if token is set
    },
