...
const captureAndResize = async (scale = 1, silent = false, mouse = false) => {
  try {
    ...
    let cursorPath = path.join(__dirname, "resources", "cursor.png");
    // Write cursor image to tmp file
    if (process.pkg) {
        // Read the PNG from the snapshot as a Buffer
        const content = fs.readFileSync(cursorPath);

        // Write the buffer to a real file in the temp directory
        const tempPath = path.join(os.tmpdir(), "cursor.png");
        fs.writeFileSync(tempPath, content);

        // Use the extracted file for reading
        cursorPath = tempPath;
    }
