...
description: "Convert user stories from CSV files into individual TestDriver.ai test files using Node.js and GitHub Actions."
icon: "table"
---

This guide demonstrates how to convert user stories from **CSV files** into individual TestDriver.ai test files using **Node.js**. Each user story will be saved as its own YAML test file, and the tests will be executed in parallel using a **GitHub Actions workflow**.
...
Store your TestDriver.ai API key as a GitHub secret (e.g., `TESTDRIVER_API_KEY`) to securely authenticate your tests.
