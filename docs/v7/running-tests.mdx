...
```bash
vitest run --maxWorkers=8
```

The `--maxWorkers` flag limits how many test files run simultaneously. This should match your TestDriver license slots to avoid failures from exhausted slots.

### Worker Pool Configuration

```bash
vitest run --pool=forks --maxWorkers=16
```

Fine-tune worker allocation for optimal performance:
- `--pool=forks` — Each worker is a separate process (**required** for TestDriver, and the Vitest default)
- `--maxWorkers` — Maximum workers to spawn (match your license slots and CPU cores)
- `--minWorkers` — Minimum number of workers to keep alive (reduces startup overhead)

<Warning>
  `pool: "threads"` is not supported. TestDriver requires `pool: "forks"` for process isolation.
</Warning>

<Info>
  For detailed concurrency tuning, including CI runner sizing and sharding, see the [Concurrency guide](/v7/concurrency).
</Info>
