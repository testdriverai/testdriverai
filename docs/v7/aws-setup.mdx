...
Tests work the same way as cloud-hosted tests:
...
<Tip>
  The `context.ip` is automatically set by the `setup-aws` hook. If you provide `TD_IP` manually, it will use that instead of spawning a new instance.
</Tip>
