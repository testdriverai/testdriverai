...
test('file menu operations', async (context) => {
  ...
  await testdriver.assert('New document window opens');
});
```
...
test('test chrome and vscode together', async (context) => {
  ...
  // Provision Chrome browser
  await testdriver.provision.chrome({ url: 'https://example.com' });
  await testdriver.find('link').click();
  
  // Can switch to VS Code in the same test
  await testdriver.provision.vscode({ workspace: '/tmp/project' });
...
test('lifecycle example', async (context) => {
  // 1. Setup happens when you create TestDriver and provision
  const testdriver = TestDriver(context, { headless: true });
  await testdriver.provision.chrome({ url });
