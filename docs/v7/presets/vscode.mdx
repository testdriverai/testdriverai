...
test('extension test', async (context) => {
  ...
  await testdriver.find('Python file type').click();
});
```
...
test('eslint and prettier integration', async (context) => {
  ...
  await testdriver.find('Open File').click();
  await testdriver.type('src/index.js');
  await testdriver.pressKeys(['enter']);
  
  // Format document
  await testdriver.pressKeys(['shift', 'alt', 'f']);
  
  await testdriver.assert('Document is formatted');
});
```
...
test('empty workspace welcome screen', async (context) => {
  ...
  await testdriver.find('Open Folder button').click();
  await testdriver.type('/tmp/my-project');
  await testdriver.pressKeys(['enter']);
  
  await testdriver.assert('Folder is opened');
});
```
...
test('windows vscode test', async (context) => {
  ...
  await testdriver.find('Explorer view').click();
});
