...
<Steps>
  <Step title="Create a TestDriver Account">
    <Card title="Sign Up" icon="sparkles" iconType="regular" href="https://app.testdriver.ai" cta="Sign Up Now">
      - 100% free
    </Card>
    Becaue TestDriver is powered by agentic users, an account is required to access:
  ...
  <Step title="Set Up TestDriver">
    ...
    <Tabs>
      <Tab title="Hosted Runners (Recommended)">
        During `init`, choose "yes" for the sandbox option. This will prompt you for your API key:
      ...
      <Tab title="Local Runner (Windows)">
  ...
  <Step title="Create your first test">
    ...
    <Tip>
      Use Control \+ C in the CLI or the square "stop" button in the extension to
      stop test execution.
    </Tip>
  </Step>
  <Step title="Running tests">
    ...
    ```bash
    npx testdriverai@latest run testdriver/your_test.yml
    ```
  ...
  <Step title="Generate self-driving tests">
    ...
    <Card title="Self-Driving Test Guide" icon="link" href="/getting-started/generating">
  ...
  <Step title="Deploy to CI/CD">
    ...
    <Card title="GitHub Actions" icon="link" href="/action/setup">
