...
description: "Learn how TestDriver.ai simplifies test generation from CSV files, Jira tickets, Gherkin scenarios, TestRail, and AI-driven exploratory testing."
---

TestDriver.ai provides a powerful and flexible framework for generating tests from various sources, including **CSV files**, **Jira tickets**, **Gherkin scenarios**, and **TestRail test cases**. Additionally, TestDriver.ai can autonomously generate its own tests by recursively mapping the UI and exploring workflows.

---

## 1. **Test Generation from CSV Files**
...
For a login page, the AI might generate the following test:```yaml
version: 4.2.18
steps:
  - prompt: Open the login page
  - prompt: Enter "test@example.com" in the email field
  - prompt: Enter "password123" in the password field
  - prompt: Click the "Log In" button
  - prompt: Verify that the dashboard is displayed
