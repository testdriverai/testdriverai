...
This allows TestDriver to locate the target for `hover-text` based on its context and description. The agent will search for elements in the following order :
...
<Note>
  Need to add `--heal` flag during the run command to enable auto-healing of the tests.
  Refer the [Auto-Healing](/getting-started/running#auto-healing) section to know more.
</Note>
