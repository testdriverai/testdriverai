...
To enable auto-healing in TestDriver, simply add the `--heal` flag when running your tests. This instructs TestDriver to attempt to automatically update any failing tests due to minor changes in the application.

```bash
testdriver run my-tests.yaml --heal
```
    ...
    steps:
      ...
      - name: Run TestDriver
        ...
        with:
          ...
          prompt: |
            1. /run testdriver/onboarding.yaml --save --heal
          create-pr: true
