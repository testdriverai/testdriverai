...
<Steps>
  <Step title="Initialize Playwright">
    ...
    <Tabs>
      <Tab title="npm">
        ```bash
        npm init playwright@latest --yes -- --lang=ts --quiet --browser=chromium --install-deps --no-github-actions .
        ```
      </Tab>
      <Tab title="yarn">
        ```bash
        yarn create playwright --yes -- --lang=ts --quiet --browser=chromium --install-deps --no-github-actions .
        ```
      </Tab>
      <Tab title="pnpm">
        ```bash
        pnpm create playwright --yes -- --lang=ts --quiet --browser=chromium --install-deps --no-github-actions .
        ```
      </Tab>
    </Tabs>
    Wait for the installation to complete, this might take a min or two as it would be installing the chromium browser.
