...
When executing the `hover-image` and `hover-text` command, TestDriver may use the description to identify the image, espeically if multiple images are visible. If multiple images on the screen match the description, TestDriver may not be able to determine which one to interact with. To improve accuracy, use specific and unique descriptions for images.
