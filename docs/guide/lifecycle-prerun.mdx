...
graph TD
    ...
    A -->|No| C{Is prerun available at ./lifecycle/prerun.yaml and a testfile present in ./somedirectory?}
    C -->|Yes| D[Run ./lifecycle/prerun.yaml]
    D --> E[Run ./somedirectory/testfile.yaml]
    C -->|No| F{Is ./somedirectory/testfile.yaml present?}
    F -->|Yes| G[Run prerun from lifecycle folder]
    G --> E
```
---
