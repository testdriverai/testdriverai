...
```bash
export TESTDRIVER_API_KEY="your_api_key_here"
```
...
```json
{
  ...
  "mcpServers": {
    "testdriver-cloud": {
      "type": "http",
      "baseUrl": "https://your-api-host.example.com/api/v1/mcp",
      "requestHeaders": {
        "X-Api-Key": "${TESTDRIVER_API_KEY}"
      },
...
- Use a local team or user API key in `TESTDRIVER_API_KEY`
...
```json
{
  "mcpServers": {
    "testdriver-cloud-local": {
      "type": "http",
      "baseUrl": "http://localhost:1337/api/v1/mcp",
      "requestHeaders": {
        "X-Api-Key": "${TESTDRIVER_API_KEY}"
      }
    }
...
- The recommended two-file workflow (`setup.test.mjs` + `experiment.test.mjs`)
