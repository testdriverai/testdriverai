...
import Replay from '/snippets/tests/scroll-until-image.mdx'
...
<Note>
Use either the `description` or `path` argument to match an image on the screen.  
If you are using `path` argument, make sure to upload an accurate representation of the actual image and reference it relative to the current test path.
</Note>
