...
## Description
The `/try` command is used to execute a single step in a test plan and add it to a specified file. This command generates and performs steps in an interactive way while creating a test plan.
...
## Example Usage
```bash
> /try add an item to the shopping cart
```
...
## Protips
- Use descriptive prompts for higher success with generating test steps with `/try`.
- Use `/try` multiple times in succession to build a comprehensive test plan.

## Gotchas
- The AI will retry 3 times if it can't complete the task.

## Notes
- The `/try` command is ideal for creating test plans in an interactive session.
