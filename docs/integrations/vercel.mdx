...
<Steps>
  <Step title="Vercel GitHub integration setup">
    Ensure your repository is connected to Vercel for automatic deployments.
  </Step>
  <Step title="Store TestDriver API key as a secret">
    Store your TestDriver API key as a GitHub secret (for example,
    `TD_API_KEY`).
  </Step>
  <Step title="Configure Vercel deployment URL">
    Use the `VERCEL_URL` environment variable provided by Vercel to access the
    deployment.
  </Step>
</Steps>
...
<Steps>
  <Step title="Wait for Vercel deployment">
    The `amondnet/vercel-action` waits for the Vercel deployment to complete and
    retrieves the deployment URL. This URL is stored in the
    `steps.vercel.outputs.url` variable.
  </Step>
  <Step title="Run tests with TestDriver">
    The TestDriver GitHub Action runs tests on the deployed application using
    the deployment URL. The `prompt` field specifies the test steps to execute.
  </Step>
  <Step title="Review test results">
    View test results in the GitHub Actions dashboard or as comments on the pull
    request.
  </Step>
</Steps>
